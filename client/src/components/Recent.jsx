...
export default function Recent() {
  ...
  return (
    <div className="space-y-4">
      {thoughts.length === 0 && <p className="text-gray-500 text-sm">No thoughts yet.</p>}
      {thoughts.map((t) => (
        <div key={t.id} className="p-4 bg-gray-800 rounded-lg">
          <div className="flex justify-between items-start mb-3">
            ...
            <button
              ...
              className="ml-3 px-3 py-1 bg-red-900/40 hover:bg-red-800 text-red-400 text-xs rounded flex items-center gap-1 shrink-0"
            >
          ...
          <div className="space-y-2 text-xs">
            {t.metadata?.type && (
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 bg-gray-700 text-gray-300 rounded">{t.metadata.type}</span>
              </div>
            )}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex gap-1 flex-wrap">
                  {t.metadata.topics.map((topic) => (
                    <span key={topic} className="px-2 py-0.5 bg-indigo-900 text-indigo-300 rounded">{topic}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <p className="text-xs text-gray-500 mt-3">{new Date(t.created_at).toLocaleString()}</p>
        </div>
      ))}
    </div>
  );
