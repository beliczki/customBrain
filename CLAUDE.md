...
## Versioning
Semver (`major.minor.patch`), currently `0.14.0`. Versions sync across root `package.json`, `server/package.json`, `client/package.json`, `extension/manifest.json`. Bump all four together and log the change in `CHANGELOG.md`. `0.x.y` = pre-1.0, breaking changes allowed on minor bumps.
...
> Suggested bump: `0.14.0` → `0.13.1` (patch). Reason: <one sentence>.

Heuristic for the suggestion (at `0.x.y`):
- **patch** (`0.14.0` → `0.13.1`): bug fix, doc-only change, internal refactor with no behaviour change, env var rename with backwards-compatible fallback.
- **minor** (`0.14.0` → `0.14.0`): new feature or capture path, new MCP tool, new HTTP route, new Qdrant payload field, payload migration, or any user-visible behaviour change. Breaking changes are allowed on minor bumps while pre-1.0.
