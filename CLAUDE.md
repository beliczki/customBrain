...
- **Capture pipeline**: text → parallel [embedding + metadata extraction] → Qdrant upsert. Context-aware: `server/drive-context.js` reads People/Projects from Drive. `server/context.json` has `not_people` exclusions. Near-duplicate detection (cosine > 0.85): archives old thought (`status: archived`), links new thought via `supersedes`. Semantic contradiction detection is limited — embeddings measure topic similarity, not logical opposition.
