...
**Ops, same day (no code change):** the dead `thoughts` collection was dropped from the Qdrant instance, closing the note left in 0.20.0 ("drop it manually once confident") four months ago. Its continued existence is what made the 0.41.2 backup bug survivable-looking — a wrong collection name still found a real collection. Verified redundant before dropping: of its 596 points, 71 were absent from `thoughts_v2` and **all 71 were chunks whose parent thought is present in v2 and already re-chunked there** — zero non-chunk points missing, so no source content existed only in the old collection. A snapshot was archived outside the rotation window at `backups/archive/thoughts-final-pre-drop-2026-09-12.snapshot.keep` (`rotateLocal` sweeps only `.snapshot` files in the backups root, not subdirectories — confirmed by a later backup run). After the drop: `/stats` 491 thoughts, `/search` returns real hits, the backup cron snapshots `thoughts_v2 (3802 points)`.
