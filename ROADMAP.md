...
- Evaluator v0 DONE + **first batch GRADED (2026-07-18)** — `tasks/evaluator/baseline-findings-2026-07-18.md`. **Top lever discovered: MS Teams is not ingested** — flagship project (Bizi/Dasszisztens) current state + the BTS-offer-declined signal live only in un-ingested Teams chats; no retrieval fix applies to a missing source. Recommend manual-capture v1 (not puppeteer), then project auto-update. Retrieval tuning stays deferred. Secondary: Haiku folds BTS/Hello Szülő into Bizi (entity confusion); MessagingMatrix↔ERSTE centrality not modeled; Attraction breaking down (flag to close).
